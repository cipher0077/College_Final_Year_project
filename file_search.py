...
class Search:
    ...
    def get_text_only(self, textdata, correction=True):
        ...
        if correction:
            ...
            if anychange:
                # confirm = input("Did you mean : "+refinedquery + " ? Enter 'y' to confirm...")
                confirm = 'n'
                if confirm == 'y':
...
def main_search(search_str):
    ...
    while permission:
        ...
        if inpq != 'y':
            permission = False
    return [f_list, sc_list, ln_list, lc_list, dinfo_list, did_list, foundValue]
...
@app.route('/random_files_to_search', methods=["POST", "GET"])
def show_file():
    filename = request.form['fname']
    fhand = open('./random_files_to_search/'+filename, 'r', encoding='utf8')
    flines = []
    for line in fhand:
        flines.append(line)
    return render_template('/readfiles.html', flist=flines,filen = filename)


@app.route('/result/<name>')
def result(name):
    temp_search = main_search(name)
    return render_template('/results.html', com=temp_search, searched_txt=name)
